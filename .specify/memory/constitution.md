...
### II. Type Safety
Strict TypeScript is enforced across the codebase. Avoid `any` — use `unknown` with type guards when the type is truly dynamic. All function signatures must have explicit parameter types. Shared interfaces live in dedicated type files.
