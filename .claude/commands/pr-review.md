...
2. **Check coding standards**
   - Verify all errors are handled (no `_` for error returns).
   - Check that `context.Context` is the first parameter where needed.
   - Confirm standard Go conventions (gofmt compliance).

3. **Check for security issues**
   - Check for `fmt.Sprintf` in SQL queries — use parameterized queries.
   - Verify `html/template` over `text/template` for HTML output.
   - Check for `os/exec` with user-controlled input.
